...
//    @Query(value = "select new " +
//            "com.turkcell.spring.starter.entities.dtos.productDto.ProductForUpdateDto(p.productId, p.productName, p.quantityPerUnit ,p.unitPrice, p.unitInStock, p.unitOnOrder, p.reorderLevel) from Product p where p.productId=:productId")
